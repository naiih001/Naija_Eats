  ...
  try {
    const newPlan = await prisma.$transaction(async (tx) => {
      ...
      const [budget, household, preferences, dietaryRestrictions] = await Promise.all([
        tx.budgets.findUnique({ where: { user_id: user.id } }),
        tx.household_profiles.findUnique({ where: { user_id: user.id } }),
        tx.user_preferences.findMany({ where: { user_id: user.id } }),
        tx.dietary_tags.findMany({ where: { user_id: user.id } }),
      ]);
      ...
      let pool = await getFilteredMeals(tx, filterOpts);

      if (pool.length < 7) {
        pool = await getFilteredMeals(tx, {
          restrictionTags: restrictionTags.length > 0 ? restrictionTags : undefined,
          includeDessert,
          perMealMax,
        });
      ...
      if (pool.length < 7) {
        pool = await getFilteredMeals(tx, { includeDessert, perMealMax });
      }
      ...
      // 6. Determine slots based on daily_meals
      ...
      // 7. Categorize meals for slot-appropriate assignment
